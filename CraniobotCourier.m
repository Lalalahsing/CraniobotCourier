...
% Last Modified by GUIDE v2.5 03-May-2021 11:12:09
...
try
    ...
    if get(hObject,'Value')
        ...
        if get(device,'Status') ~= 'open'
            ...
            set(handles.suctionSpiralMenu,'enable','on');
            set(handles.suctionFlowerMenu,'enable','on');
    ...
    % Else, if button is switched "off"
        ...
        set(handles.suctionSpiralMenu,'enable','off');
        set(handles.suctionFlowerMenu,'enable','off');
...
figHandles.depthLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Depth per Path (mm)',...
    'Position',[5,210,150,20]);
figHandles.depthTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','depthTextBox',...
    'String','0.05',...
    'Position',[200,210,50,20]);
...
figHandles.VesselWidthTextBox = uicontrol(fig,'Style','edit',...
    ...
    'String','0.5',...
    'Position',[200,120,50,20]);
figHandles.VesselHeightLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Vessel Height (mm)',...
    'Position',[5,90,150,20]);
figHandles.VesselHeightTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','heightTextBox',...
    'String','0.5',...
    'Position',[200,90,50,20]);
figHandles.feedrateVesselLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Vessel Feedrate (mm/min)',...
    'Position',[5,60,150,20]);
figHandles.feedrateVesselTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'String','10',...
    'Position',[200,60,50,20]);
figHandles.millSkullButton = uicontrol(fig,'Style','pushbutton',...
    'Units','pixels',...
    'Position',[30,10,200,40],...
...
if isempty(handles.skullPoints)
    ...
    VesselHeight = str2double(get(figHandles.VesselHeightTextBox, 'String'));
    feedrateV    = str2double(get(figHandles.feedrateVesselTextBox,'String'));
    feedrate     = str2double(get(figHandles.feedrateTextBox,'String'));
    
    if isempty(depth) || isempty(thickness) || isempty(feedrate) || isempty(VesselWidth) || isempty(VesselHeight) || isempty(feedrateV)
        uiwait( errordlg('Missing input parameter...',...
                         'Input Error', 'modal') );
    else
        close;
        millProbedPoints(xVals,yVals,zVals,thickness,VesselWidth,VesselHeight,depth,feedrate,feedrateV);
    end
end
end
%% File Manager
...
set(findall(handles.MotionButtonsGrp, '-property', 'enable'), 'enable', 'off');
set(findall(handles.commonCommandsGrp, '-property', 'enable'), 'enable', 'off');
set(handles.commandLine,'enable','off');
...
% --------------------------------------------------------------------
function suctionSpiralMenu_Callback(~, ~, ~)
% hObject    handle to suctionSpiralMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%guidata(hObject,handles);
% create new window
fig = figure('Name','Suction Parameters',...
    'Units','pixels',...
...
figHandles.spiralSkullButton = uicontrol(fig,'Style','pushbutton',...
    ...
    'String','Generate Suction Program',...
    'Callback',@suctionSpiralButton_Callback);

guidata(fig,figHandles);
end
function suctionSpiralButton_Callback(~, ~, ~)
% Objective: Take the recorded skull points and generate a milling filling
% accordingly
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
figHandles = guidata(findobj(0,'Tag','sucWindow')); % get handles 
handles    = guidata(findobj(0,'Tag','GUI'));
...
%    zVals       = handles.skullPoints(:,3);
    ...
    if isempty(suckVal) || isempty(diaVal) isempty(depth) || ...
        ...
        close;
        suctionSpiral(suckVal,diaVal,xVal,yVal,thickness,depth,feedrate);
    end
end

% --------------------------------------------------------------------
function suctionFlowerMenu_Callback(~, ~, ~)
% hObject    handle to suctionFlowerMenu (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%guidata(hObject,handles);
% create new window
...
figHandles = guidata(fig);
% create window elements
figHandles.SchamberLocationFLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Chamber Location (X,Y) (mm)',...
    'Position',[5,180,150,20]);
figHandles.SchamberXFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','SchamberXSTextBox',...
    'String','2.2',...
    'Position',[200,180,50,20]);
figHandles.SchamberYFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','SchamberYSTextBox',...
    'String','-1',...
    'Position',[260,180,50,20]);
figHandles.SchamberDiameterFLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Chamber/Niddle Diameter (mm)',...
    'Position',[5,150,150,20]);
figHandles.SchamberDiameterFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','chamberDiameterTextBox',...
    'String','1.4',...
    'Position',[200,150,50,20]);
figHandles.SchamberSuckFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'String','0.65',...
    'Position',[260,150,50,20]);
figHandles.SdepthFLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Depth per Pass (mm)',...
    'Position',[5,120,150,20]);
figHandles.SdepthFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','depthTextBox',...
    'String','0.2',...
    'Position',[200,120,50,20]);

figHandles.SthicknessFLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Skull Thickness (mm)',...
    'Position',[5,90,150,20]);
figHandles.SthicknessFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'Tag','thicknessTextBox',...
    'String','2',...
    'Position',[200,90,50,20]);

figHandles.SfeedrateFLabel = uicontrol(fig,'Style','text',...
    'Units','pixels',...
    'String','Feedrate (mm/min)',...
    'Position',[5,60,150,20]);
figHandles.SfeedrateFTextBox = uicontrol(fig,'Style','edit',...
    'Units','pixels',...
    'String','20',...
    'Position',[200,60,100,20]);

figHandles.spiralSkullFButton = uicontrol(fig,'Style','pushbutton',...
    'Units','pixels',...
    'Position',[100,10,200,40],...
    'String','Generate Suction Program',...
    'Callback',@suctionFlowerButton_Callback);

guidata(fig,figHandles);
end
function suctionFlowerButton_Callback(~, ~, ~)
% Objective: Take the recorded skull points and generate a milling filling
% accordingly
...
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
figHandles = guidata(findobj(0,'Tag','sucFWindow')); % get handles 
handles    = guidata(findobj(0,'Tag','GUI'));

    suckVal     = str2double(get(figHandles.SchamberSuckFTextBox,'String'));
    diaVal      = str2double(get(figHandles.SchamberDiameterFTextBox,'String'));
%    xVals       = handles.skullPoints(:,1);
%    yVals       = handles.skullPoints(:,2);
%    zVals       = handles.skullPoints(:,3);
    depth       = str2double(get(figHandles.SdepthFTextBox,'String'));
    thickness   = str2double(get(figHandles.SthicknessFTextBox,'String'));
    feedrate    = str2double(get(figHandles.SfeedrateFTextBox,'String'));
    xVal        = str2double(get(figHandles.SchamberXFTextBox,'String'));
    yVal        = str2double(get(figHandles.SchamberYFTextBox,'String'));
    ...
    if isempty(suckVal) || isempty(diaVal) isempty(depth) || ...
        ...
        close;
        suctionFlower(suckVal,diaVal,xVal,yVal,thickness,depth,feedrate);
    end
end
